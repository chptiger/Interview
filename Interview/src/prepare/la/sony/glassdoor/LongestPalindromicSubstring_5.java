...
/**
 * A palindrome is a string which reads the same in both directions. For
 * example, SS = "aba" is a palindrome, SS = "abc" is not.
 * 
 ...
 * https://www.cnblogs.com/yuzhangcmu/p/4189068.html 详细图解：
...
public class LongestPalindromicSubstring_5 {
	 ...
	 * 我们维护一个二维数组dp，其中dp[i][j]表示字符串区间[i, j]是否为回文串， 当i = j时，只有一个字符，肯定是回文串， 如果i = j +
	 * 1，说明是相邻字符，此时需要判断s[i]是否等于s[j]， 如果i和j不相邻，即i - j >= 2时，除了判断s[i]和s[j]相等之外，dp[j +
	 * 1][i - 1]若为真，就是回文串， 通过以上分析，可以写出递推式如下：
	 * 
	 * dp[i, j] = 1 if i == j
	 * 
	 * = s[i] == s[j] if j = i + 1
	...
	private static int expandAroundCenter(char[] input, int l, int r) {
		...
		return end - start - 1;
		// why did not return end - start? we will start from i, try to get the length
		// between i and start, i and end.
	}
}
