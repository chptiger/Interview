...
public class TargetSum_494 {
	...
	/**
	 * we should put count() into the else {} 1. if arr length is 1, there will be
	 * ArrayOutOfBoundsException for second count() method
	 * 
	 * 
	 */
	private static void count_wrong(int[] arr, int index, int sum, int target) {
		if (index == arr.length) {
			if (sum == target) { // if(index == arr.length && sum == target) we could not return in the final
									// level
				count++;
			}
			return;
		...
		count(arr, index - 1, sum - arr[index], target); // it should index + 1 2. if arr length larger than 1, there
															// will be StackOverFlowError the method will alwarys run
															// two count() methods.
	}

	// if else or add return to make current method complete to avoid
	// StackOverFlowError.
	private static void count(int[] arr, int index, int sum, int target) {
		if (index == arr.length) {
			if (sum == target)
				count++;
			// return; // do not need return statement there. we use if else, the current
			// count method was completed.
		} else {
			count(arr, index + 1, sum + arr[index], target);
			count(arr, index + 1, sum - arr[index], target);
		}
